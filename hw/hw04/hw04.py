def shuffle(s):
    ...
    result = [(l if i % 2 == 0 else r)[i//2] for i in range(len(s))]
    return result
...
def deep_map(f, s):
    ...
    """
    "*** YOUR CODE HERE ***"
    def deep_map_copy(f, s):
        return [deep_map_copy(f, x) if isinstance(x, list) else f(x) for x in s]
    def deep_map_inplace(f, s):
        for i in range(len(s)):
            if isinstance(s[i], list):
                deep_map_inplace(f, s[i])
            else:
                s[i] = f(s[i])
    deep_map_inplace(f, s)
...
def balanced(m):
    ...
    return balanced(end(left_arm)) and balanced(end(right_arm)) and \
           length(left_arm) * total_mass(end(left_arm)) == length(right_arm) * total_mass(end(right_arm))
...
def max_path_sum(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    return label(t) if is_leaf(t) else label(t) + max([max_path_sum(b) for b in branches(t)])
