...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    selection = [paragraph for paragraph in paragraphs if select(paragraph)]
...
def about(keywords):
    ...
    def select(paragraph):
        """Return whether the paragraph contains one of the keywords."""
        paragraph_words = split(remove_punctuation(lower(paragraph)))
        return any([keyword in paragraph_words for keyword in keywords])
    return select
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if len_typed == 0 or len_source == 0:
        return 0.0
    correct = sum(1 for i in range(min(len_typed, len_source)) if typed_words[i] == source_words[i])
    return (correct / len_typed) * 100.0 if len_typed > 0 else 0.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    closest_word, similar_diff = min(zip(word_list, words_diff), key=lambda item: item[1])
