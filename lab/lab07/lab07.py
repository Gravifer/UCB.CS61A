...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        while projected_balance < amount:
            projected_balance += projected_balance * self.interest
            years += 1
        return years
...
class FreeChecking(Account):
    ...
    def __init__(self, account_holder):
        ...
        self.free_withdrawals = self.__class__.free_withdrawals
        self.withdraw_fee = self.__class__.withdraw_fee
    def withdraw(self, amount):
        if self.free_withdrawals > 0:
            self.free_withdrawals -= 1
            if amount > self.balance:
                return "Insufficient funds"
        elif amount > self.balance + self.withdraw_fee:
            return "Insufficient funds"
        else:
...
def duplicate_link(s, val):
    ...
    if s.rest is Link.empty: # reached the end of the list
        if s.first == val:
            s.rest = Link(val)
        return
    duplicate_link(s.rest, val) # recurse to the end of the list
    if s.first == val: # this is the one to duplicate
